...
import psych_core.objects.MenuItem;
import psych_core.objects.MenuCharacter;

import psych_core.options.GameplayChangersSubstate;
import psych_core.substates.ResetScoreSubState;
...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(WeekData.weeksList.length < 1)
		{
			...
			MusicBeatState.switchState(new psych_core.states.ErrorState("NO WEEKS ADDED FOR STORY MODE\n\nPress ACCEPT to go to the Week Editor Menu.\nPress BACK to return to Main Menu.",
				function() MusicBeatState.switchState(new psych_core.states.editors.WeekEditorState()),
				function() MusicBeatState.switchState(new psych_core.states.MainMenuState())));
			return;
		}
	...
	function selectWeek()
	{
		if (!weekIsLocked(loadedWeeks[curWeek].fileName))
		{
			...
			try
			{
				...
				trace('ERROR! $e');
				MusicBeatState.switchState(new psych_core.states.ErrorState('Error loading week: "' + loadedWeeks[curWeek].fileName + '"', e));
				return;
			}
