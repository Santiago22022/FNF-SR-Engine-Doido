package psych_core.states.stages.objects;
...
class ABotSpeaker extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0)
	{
		...
		try {
			// Debug: check if anim loaded
			if (eyes.anim == null) {
				trace("ERROR: eyes.anim is NULL for abot/systemEyes");
			} else {
				// trace("Loaded symbols: " + eyes.anim.symbolDictionary.keys()); // Can be verbose
			}
			...
			add(eyes); // Only add if successful
		} catch(e:Dynamic) {
			trace("CRASH PREVENTED: Failed to load ABot eyes animation: " + e);
			// Do not add to group.
		}
		...
		try {
			...
			add(speaker); // Only add if successful
		} catch(e:Dynamic) {
			trace("CRASH PREVENTED: Failed to load ABot speaker animation: " + e);
		}
	}
	...
	override function update(elapsed:Float):Void
	{
		...
		{
			//trace(levelMax);
			if(oldLevelMax <= levelMax && (levelMax >= 5 || (speaker.visible && speaker.anim != null && speaker.anim.curFrame >= 3)))
				beatHit();
		}
	}
	#end
	...
	{
		if (speaker == null || !speaker.visible) return;
		speaker.anim.play('anim', true);
	}
	...
	{
		if (eyes == null || !eyes.visible) return;
