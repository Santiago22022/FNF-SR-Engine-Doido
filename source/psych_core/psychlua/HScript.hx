...
#if LUA_ALLOWED
import psych_core.psychlua.FunkinLua;
#end
...
class HScript extends Iris
{
	...
	private static function patchCode(code:String):String {
		if(code == null) return null;
		var namespaces:Array<String> = ["backend", "cutscenes", "debug", "external", "flxanimate", "objects", "options", "psychlua", "shaders", "states", "substates", "unused"];
		for(ns in namespaces) {
			var r = new EReg("import\\s+" + ns + "\\.", "g");
			code = r.replace(code, "import psych_core." + ns + ".");
			
			var r2 = new EReg("using\\s+" + ns + "\\.", "g");
			code = r2.replace(code, "using psych_core." + ns + ".");
		}
		return code;
	}
	...
	override public function new(?parent:Dynamic, ?file:String, ?varsToBring:Any = null, ?manualRun:Bool = false)
	{
		...
		#end
		super(patchCode(scriptThing), new IrisConfig(scriptName, false, false));
		...
		set('addHaxeLibrary', function(libName:String, ?libPackage:String = '') {
			try {
				...
				if(libPackage.length > 0) {
					var namespaces:Array<String> = ["backend", "cutscenes", "debug", "external", "flxanimate", "objects", "options", "psychlua", "shaders", "states", "substates", "unused"];
					if(namespaces.contains(libPackage.split('.')[0])) libPackage = 'psych_core.' + libPackage;
					str = libPackage + '.';
				}
